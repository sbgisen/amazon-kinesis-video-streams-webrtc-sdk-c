...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    if (argc < 3) {
        DLOGE("Usage: %s %s <gst-command>\n", argv[0], argv[1]);
        return EXIT_FAILURE;
    }

    for (int i = 2; i < argc; ++i) {
        strcat(pSampleConfiguration->gstreamerCommand, argv[i]);
        strcat(pSampleConfiguration->gstreamerCommand, " ");
    }

    DLOGI("[KVS GStreamer Master] Command: %s", pSampleConfiguration->gstreamerCommand);
